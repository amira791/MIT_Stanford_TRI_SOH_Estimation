...
def build_sequences(df: pd.DataFrame, scaler: StandardScaler, 
                    seq_len: int = SEQ_LEN, 
                    horizon: int = PREDICTION_HORIZON) -> TensorDataset:
    """
    Build sliding window sequences for each cell.
    
    Input: cycles [t, t+1, ..., t+seq_len-1] (seq_len cycles)
    Target: SOH at cycle t+seq_len+horizon-1 (horizon cycles ahead)
    
    Example with seq_len=50, horizon=50:
        Input: cycles 1-50
        Target: SOH at cycle 100 (50 cycles after the window)
    """
    X_list, y_list = [], []
    ...
    for cell_id, grp in df.groupby("cell_id"):
        ...
        n = len(grp)
        # Need enough cycles for window + horizon
        if n <= seq_len + horizon:
            continue

        for i in range(n - seq_len - horizon + 1):
            X_list.append(X_cell[i:i + seq_len])
            # Target is horizon steps ahead of the window end
            y_list.append(y_cell[i + seq_len + horizon - 1])
