# train_nca.py
# Train BEM-SOH from scratch on SNL NCA dataset
# Purpose: Test architecture generalization to NCA chemistry
#
# CRITICAL: Uses EXACTLY the same hyperparameters as MIT and NCM
# This ensures a fair architecture generalization test
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. Config — IDENTICAL to MIT and NCM
# ─────────────────────────────────────────────────────────────────────────────
...
# ─── NCA FEATURES (8 features, same as NCM) ───
NCA_FEAT_COLS = [
    "charge_capacity",
    "charge_energy",
...
CFG = dict(
    ...
    # ─── TRAINING (IDENTICAL to MIT and NCM) ───
    soh_epochs   = 120,
    soh_lr       = 2e-4,
    soh_batch    = 256,
    soh_wd       = 1e-4,
    ...
    # ─── EVIDENTIAL (IDENTICAL to MIT and NCM) ───
...
class NCASequenceDataset(Dataset):
    def __init__(self, df, window_size, stride=1, split=None,
        ...
        for cid, cell_df in subset.groupby("cell_id"):
            ...
            X = cell_df[NCA_FEAT_COLS].values.astype(np.float32)
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. Model (IDENTICAL to MIT and NCM)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. Losses (IDENTICAL to MIT and NCM)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. Training (IDENTICAL to MIT and NCM)
# ─────────────────────────────────────────────────────────────────────────────
...
def train_soh(model, train_ds, val_ds, cfg):
    print("\n" + "=" * 60)
    print("  TRAINING BEM-SOH ON NCA")
    print("  (Architecture Generalization Test)")
    print("=" * 60)
...
# ─────────────────────────────────────────────────────────────────────────────
# 6. Evaluation (IDENTICAL to MIT and NCM)
# ─────────────────────────────────────────────────────────────────────────────
...
def evaluate_soh(model, val_loader, test_loader, cfg):
    print("\n" + "=" * 60)
    print("  SOH EVALUATION - NCA TEST SET")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    print(f"Hyperparameters: IDENTICAL to MIT and NCM")
    ...
    scaler.fit(df[df.split == "train"][NCA_FEAT_COLS].values)
    df[NCA_FEAT_COLS] = scaler.transform(df[NCA_FEAT_COLS].values)

    # ─── Create datasets ───
    W = CFG["window_size"]
    train_ds = NCASequenceDataset(df, W, CFG["soh_stride"], "train",
                                   weighted=True, tail_weight=CFG["tail_weight"])
    val_ds = NCASequenceDataset(df, W, CFG["soh_stride"], "val",
                                 weighted=True, tail_weight=CFG["tail_weight"])
    test_ds = NCASequenceDataset(df, W, CFG["soh_stride"], "test")
    ...
    print("\n" + "=" * 60)
    print("  FINAL RESULTS SUMMARY - NCA")
    print("=" * 60)
    print(json.dumps({**results, "total_params": total_params}, indent=2, default=str))
    print("=" * 60)