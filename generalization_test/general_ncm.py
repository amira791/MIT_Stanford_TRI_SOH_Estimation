# prepare_ncm_generalization_data.py
# 
# FIXED: Only processes *_cycle_data.csv files (not timeseries)
# Handles temperature variations gracefully
...
OUT_FILE = os.path.join(OUT_DIR, "ncm_processed.csv")

N_EARLY_CYCLES = 10
SPLIT_RATIOS = dict(train=0.7, val=0.15, test=0.15)
SPLIT_SEED = 42

# 8 features (temperature will be handled separately)
NCM_FEAT_COLS = [
    "charge_capacity",
    "charge_energy",
...
def load_cycle_files(raw_dir):
    ...
    # Only match files ending with cycle_data.csv
    ...
    if not paths:
        ...
        pattern = os.path.join(raw_dir, "*cycle_data*.csv")
        paths = glob.glob(pattern)
        paths = sorted(paths)
        print(f"  Found {len(paths)} cycle_data files (alternative pattern)")
    ...
    for i, p in enumerate(paths):
        try:
            ...
            cell_id = filename.replace('_cycle_data.csv', '').replace('_timeseries.csv', '')
...
def standardize_columns(df):
    ...
    if 'Cell_Temperature (C)' in df.columns:
        df['temperature_avg'] = df['Cell_Temperature (C)']
        print("  Found Cell_Temperature")
    elif 'Environment_Temperature (C)' in df.columns:
        df['temperature_avg'] = df['Environment_Temperature (C)']
        print("  Found Environment_Temperature")
    else:
        print("  No temperature column found. Using 25°C.")
...
if __name__ == "__main__":
    print("=" * 60)
    print("  PREPARING NCM GENERALIZATION DATA")
    print("  (Cycle Data Only)")
    print("=" * 60)
    
    print("\nLoading NCM cycle_data files...")
