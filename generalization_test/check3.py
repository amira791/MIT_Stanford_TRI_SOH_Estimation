...
def remove_capacity_artifacts(g, cell_id, log):
    ...
    Checks BOTH charge_capacity AND discharge_capacity independently - SOH
    is computed from discharge_capacity, so a discharge-only artifact must
    be caught even if that row's charge_capacity looks normal (this fixes
    the "SOH hits exactly 0.0" issue: the original check only inspected
    charge_capacity and let asymmetric discharge-only artifacts through).
    ...
    # (b) RPT/spike detection via rolling median - check both columns,
    # drop if EITHER deviates beyond threshold
...
def assign_cell_level_split(df, ratios, seed, stratify_by_protocol=True):
    ...
    print(f"  NOTE: with small per-protocol cell counts, exact 70/15/15 is not always "
          ...
          f"rather than being dumped entirely into train.")
