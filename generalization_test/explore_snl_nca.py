...
# ─────────────────────────────────────────────────────────────────────────────
# 4. Inspect a timeseries file (for temperature)
# ─────────────────────────────────────────────────────────────────────────────

print("\n" + "=" * 60)
print("  INSPECTING TIMESERIES FILE (Temperature Check)")
print("=" * 60)
...
if cycle_files:
    ...
    for i, f in enumerate(cycle_files[:10]):
        try:
            df = pd.read_csv(f)
...
# ─────────────────────────────────────────────────────────────────────────────
# 8. Check column consistency across files
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 9. Check SOH calculation feasibility
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 10. Protocol parsing from filenames
# ─────────────────────────────────────────────────────────────────────────────

print("\n" + "=" * 60)
print("  PROTOCOL PARSING (from filenames)")
print("=" * 60)
...
if cycle_files:
    ...
    for f in cycle_files:
        ...
        parts = filename.replace('_cycle_data.csv', '').split('_')
        # Look for temperature and C-rate patterns
        temp_patterns = ['15C', '25C', '35C', '45C']
        c_rate_patterns = ['0.5-0.5C', '0.5-1C', '0.5-2C', '0.5-3C']
        
        temp = next((p for p in temp_patterns if p in filename), 'unknown')
        c_rate = next((p for p in c_rate_patterns if p in filename), 'unknown')
        
        # Check DOD window
        if '0-100' in filename:
            dod = '0-100'
        elif '20-80' in filename:
            dod = '20-80'
        elif '40-60' in filename:
            dod = '40-60'
        else:
            dod = 'unknown'
        
        protocols.append(f"{temp}_{dod}_{c_rate}")
    
    protocol_counts = pd.Series(protocols).value_counts()
    print("\nProtocol groups found:")
    for proto, count in protocol_counts.items():
        print(f"  {proto}: {count} cells")
...
if cycle_files:
    ...
    # Check for temperature in timeseries
    if timeseries_files:
        df_ts_sample = pd.read_csv(timeseries_files[0])
        has_temp = any('temp' in col.lower() for col in df_ts_sample.columns)
        print(f"  Temperature in timeseries: {'✅ Yes' if has_temp else '❌ No'}")
    else:
        print("  Temperature in timeseries: ❌ No timeseries files")

print("\n✅ Exploration complete! You can now run the preprocessing script.")