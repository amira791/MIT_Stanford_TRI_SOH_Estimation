# train_snl_nca_kfold_v2.py
# K-Fold by cell training on SNL NCA dataset (9 features)
...
CFG = dict(
    # ─── UPDATED: Path to fixed NCA file ───
...
def main():
    print("=" * 60)
    print("  K-FOLD TRAINING ON SNL NCA (9 Features) - v2 (stratified + pooled OOF)")
    print("=" * 60)
    ...
    print(f"  Features: {len(NCA_FEAT_COLS)} (including temperature and voltage_range)")
    ...
    for fold_idx in range(CFG["n_folds"]):
        ...
        scaler.fit(df_fold[df_fold.split == "train"][NCA_FEAT_COLS].values)
        df_fold[NCA_FEAT_COLS] = scaler.transform(df_fold[NCA_FEAT_COLS].values)

        # Create datasets
        W = CFG["window_size"]
        train_ds = NCASequenceDataset(df_fold, W, CFG["soh_stride"], "train",
                                       weighted=True, tail_weight=CFG["tail_weight"])
        val_ds = NCASequenceDataset(df_fold, W, CFG["soh_stride"], "val",
                                     weighted=True, tail_weight=CFG["tail_weight"])
        test_ds = NCASequenceDataset(df_fold, W, CFG["soh_stride"], "test")
    ...
    save_path = os.path.join(CFG["save_dir"], "snl_nca_kfold_results_v2.json")
