# ablation_window_L20.py
# Train and evaluate BEM-SOH with window size L=20.
...
if __name__ == "__main__":
    print("=" * 60)
    print(f"  WINDOW SIZE ABLATION — L={WINDOW_SIZE}")
    print("=" * 60)
    ...
    # ─── Datasets ───
    train_ds = SequenceDataset(soh_df, WINDOW_SIZE, cfg["soh_stride"], "train",
    ...
    # ─── Save model ───
    save_path = os.path.join(cfg["save_dir"], f"bem_soh_W{WINDOW_SIZE}.pt")
    torch.save({
    ...
    # ─── Save results ───
    results_path = os.path.join(cfg["save_dir"], f"results_W{WINDOW_SIZE}.json")
