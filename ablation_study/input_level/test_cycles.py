...
def train_model(model, train_ds, val_ds, cfg):
    print("\n" + "=" * 60)
    print(f"  TRAINING (window_size={cfg['window_size']})")
    print("=" * 60)
    ...
    es.restore(model)
    print(f"  Best val loss: {es.best:.6f}")
    return model
...
def evaluate_model(model, val_loader, test_loader, cfg):
    print("\n  Evaluating...")
    ...
    results = {
        ...
        "picp_raw": picp_raw,
        "pinw_raw": pinw_raw,
        "picp_calibrated": picp_cal,
        "pinw_calibrated": pinw_cal,
...
if __name__ == "__main__":
    ...
    # ─── Load data once ───
    print("\nLoading data...")
    soh_df, scaler = load_soh_data(BASE_CFG["soh_path"])
    ...
    for W in WINDOW_SIZES:
        ...
        # ─── Train ───
        model = train_model(model, train_ds, val_ds, cfg)

        # ─── Evaluate ───
        results = evaluate_model(model, val_loader, test_loader, cfg)
        all_results.append(results)

        # ─── Display results for this window size ───
        print("\n" + "=" * 60)
        print(f"  RESULTS FOR WINDOW SIZE = {W}")
        print("=" * 60)
        print(f"  MAE:              {results['mae']:.4f}%")
        print(f"  RMSE:             {results['rmse']:.4f}%")
        print(f"  R²:               {results['r2']:.4f}")
        print(f"  PICP (raw):       {results['picp_raw']:.4f}")
        print(f"  PICP (calibrated):{results['picp_calibrated']:.4f}")
        print(f"  PINW (raw):       {results['pinw_raw']:.4f}")
        print(f"  PINW (calibrated):{results['pinw_calibrated']:.4f}")
        print(f"  Aleatoric var:    {results['mean_aleatoric']:.6f}")
        print(f"  Epistemic var:    {results['mean_epistemic']:.6f}")
        print(f"  Test windows:     {results['n_test_windows']:,}")
        print("=" * 60)

        # ─── Save model ───
        save_path = os.path.join(cfg["save_dir"], f"bem_soh_W{W}.pt")
        torch.save({
        ...
        intermediate_path = os.path.join(cfg["save_dir"], f"results_W{W}.json")
    ...
    print("=" * 60)
    print(f"\n  {'Window':<8} {'MAE (%)':<12} {'RMSE (%)':<12} {'R²':<10} "
          f"{'PICP (cal)':<12} {'PINW (cal)':<12}")
    ...
    print(f"\n  Best MAE: window_size = {best_mae['window_size']} "
          f"(MAE = {best_mae['mae']:.4f}%)")

    # ─── Save summary ───
    summary_path = os.path.join(BASE_CFG["save_dir"], "window_ablation_results.json")
