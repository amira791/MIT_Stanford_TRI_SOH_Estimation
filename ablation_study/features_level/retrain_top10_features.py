# train_soh_top10_features.py
#
# Retrains the CNN-Mamba-UQ SOH model on the TOP 10 features identified by
# permutation importance analysis, to validate if a smaller feature set
# can match or exceed the 10-feature manually selected model.
#
# TOP 10 FEATURES FROM PERMUTATION IMPORTANCE (18-feature model):
# 1. charge_capacity
# 2. cap_rel
# 3. charge_energy
# 4. energy_rel
...
# 6. initial_capacity
# 7. cycle_norm
# 8. dc_internal_resistance
# 9. date_time_iso_numeric
# 10. temperature_average
...
# ─────────────────────────────────────────────────────────────────────────────
# 1.  Feature list - TOP 10 from permutation importance
# ─────────────────────────────────────────────────────────────────────────────

FEAT_COLS = [
    "charge_capacity",          # Rank 1 - CRITICAL (1.8765% RMSE increase)
    "cap_rel",                  # Rank 2 - CRITICAL (1.6197%)
    "charge_energy",            # Rank 3 - HIGH (1.1927%)
    "energy_rel",               # Rank 4 - HIGH (0.7455%)
    "cycle_pos",                # Rank 5 - MODERATE (0.4981%)
    "initial_capacity",         # Rank 6 - MODERATE (0.3938%)
    "cycle_norm",               # Rank 7 - MODERATE (0.3391%)
    "dc_internal_resistance",   # Rank 8 - MODERATE (0.3133%)
    "date_time_iso_numeric",    # Rank 9 - LOW (0.2030%)
    "temperature_average",      # Rank 10 - LOW (0.1987%)
]
...
CFG = dict(
    ...
    save_path = r"C:\Users\admin\Desktop\DR2\16 Contributions\Contr03\MIT_Stanford_TRI_SOH_Estimation\checkpoints\soh_top10_features_best.pt",

    input_dim = len(FEAT_COLS),  # 10 - dynamically matches FEAT_COLS above
    window_size = 50,
    soh_stride = 2,
...
# ─────────────────────────────────────────────────────────────────────────────
# 4.  Model (identical architecture, only input_dim changes)
# ─────────────────────────────────────────────────────────────────────────────
...
def train_soh(model, train_ds, val_ds, cfg):
    print("\n" + "="*60)
    print("  TRAINING SOH -- TOP 10 FEATURE SET")
    print("="*60)
...
def evaluate_soh(model, test_ds, cfg):
    print("\n" + "="*60)
    print("  SOH EVALUATION — TEST SET (Top 10 Feature Model)")
    print("="*60)
...
if __name__ == "__main__":
    print("="*60)
    print("  TRAINING SOH MODEL -- TOP 10 FEATURES")
    print("="*60)
    print(f"Device: {DEVICE}")
    ...
    print("\nBuilding CNN-Mamba-UQ model (Top 10 features)...")
    ...
    print("\n" + "="*60)
    print("  FINAL RESULTS SUMMARY (Top 10 Feature Model)")
    print("="*60)
