# config_E.py
# Configuration E: Single-Scale CNN + Bidirectional + Evidential
# 
# Purpose: Tests the value of multi-scale CNN feature extraction
# Compares to Config D (Multi-scale CNN) to isolate the CNN contribution
#
# Architecture:
...
#
# What it isolates:
#   - Does multi-scale CNN matter when we already have BiMamba?
#   - Single-scale CNN removes the multi-resolution feature extraction
#   - Kernel=7 is the "middle" branch from Config D
#
# Hypothesis:
#   - If Config E ≈ Config D, multi-scale CNN is not needed
#   - If Config E > Config D, multi-scale CNN provides valuable
#     multi-resolution features that BiMamba cannot capture alone
...
CFG = dict(
    ...
    save_path = r"C:\Users\admin\Desktop\DR2\16 Contributions\Contr03\MIT_Stanford_TRI_SOH_Estimation\checkpoints\config_E_singlescale_bidirectional_evidential_30.pt",
    ...
    # Model
    # ─── CONFIG E: SINGLE-SCALE CNN ───
    cnn_channels = [128],      # Single branch (instead of [32, 64, 128])
    cnn_kernels  = [7],        # Single kernel (middle branch from Config D)
    # Config D had: [32, 64, 128] and [3, 7, 15]
    # Config E has: [128] and [7] — one branch only
    d_model      = 128,
    d_state      = 16,
    d_conv       = 4,
    expand       = 2,
    ...
    # CONFIG E: Single-scale CNN + Bidirectional + Evidential
...
class MultiScaleCNN(nn.Module):
    """Multi-scale CNN — but Config E uses only 1 branch."""
    def __init__(self, input_dim, channels, kernels, dropout=0.1):
        ...
        self.out_dim = sum(channels)  # Config E: 128 (single branch)
...
class BEM_SOH(nn.Module):
    def __init__(self, cfg):
        ...
        cnn_out = sum(C["cnn_channels"])  # Config E: 128
...
def train_soh(model, train_ds, val_ds, cfg):
    print("\n" + "=" * 60)
    print(f"  TRAINING CONFIG E: Single-Scale CNN + Bidirectional + Evidential")
    print("=" * 60)
...
def evaluate_soh(model, val_loader, test_loader, cfg):
    print("\n" + "=" * 60)
    print("  SOH EVALUATION - TEST SET (Config E: Single-Scale + Bi + Evi)")
    print("=" * 60)
...
def analytical_complexity(cfg):
    ...
    # Config E: single branch (kernel=7, channels=128) instead of 3 branches
    cnn_flops = 0
...
def print_deployment_report(model, cfg):
    print("\n" + "=" * 60)
    print("  MODEL COMPLEXITY & DEPLOYMENT METRICS (Config E)")
    print("=" * 60)
...
if __name__ == "__main__":
    print("=" * 60)
    print("  CONFIGURATION E: Single-Scale CNN + Bidirectional + Evidential")
    print("=" * 60)
    print(f"Device: {DEVICE}")
    ...
    print("\nBuilding BEM-SOH model (Config E: Single-Scale + Bi + Evi)...")
    ...
    print("\n" + "=" * 60)
    print("  FINAL RESULTS SUMMARY (Config E)")
    print("=" * 60)
