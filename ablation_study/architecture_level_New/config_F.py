# config_F.py
# Configuration F: Multi-Scale CNN + Mean-Pooling + Bidirectional + Evidential
# 
# Purpose: Tests the value of learned attention pooling
# Compares to Config D (Attention pooling) to isolate the pooling contribution
#
# Architecture:
...
#
# What it isolates:
#   - Does learned attention pooling matter when we already have BiMamba?
#   - Mean-pooling simply averages all time steps equally
#   - Attention pooling learns which cycles are most informative
#
# Hypothesis:
#   - If Config F ≈ Config D, attention pooling is not needed
#   - If Config F > Config D, attention pooling provides value by
#     focusing on informative cycles (e.g., near knee-points)
...
CFG = dict(
    ...
    save_path = r"C:\Users\admin\Desktop\DR2\16 Contributions\Contr03\MIT_Stanford_TRI_SOH_Estimation\checkpoints\config_F_multiscale_meanpool_bidirectional_evidential_30.pt",
    ...
    # Model
    # ─── CONFIG F: MULTI-SCALE CNN (SAME as Config D) ───
    cnn_channels = [32, 64, 128],   # Same as Config D
    cnn_kernels  = [3, 7, 15],      # Same as Config D
    d_model      = 128,
    d_state      = 16,
    d_conv       = 4,
    expand       = 2,
    ...
    # CONFIG F: Multi-scale CNN + Mean-Pooling + Bidirectional + Evidential
    ...
    mean_pool     = True,    # ← CONFIG F: USE MEAN-POOLING instead of attention
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  Model — WITH MEAN-POOLING SUPPORT
# ─────────────────────────────────────────────────────────────────────────────

class MultiScaleCNN(nn.Module):
    """Multi-scale CNN (same as Config D)."""
...
class BEM_SOH(nn.Module):
    def __init__(self, cfg):
        ...
        # ─── CONFIG F: MEAN-POOLING (instead of attention) ───
        # No learnable attention parameters
    ...
    def encode(self, x):
        ...
        if self.mean_pool:
            ...
            attn = None  # No attention weights to return
        else:
            # ─── ATTENTION POOLING (Config D) ───
...
def train_soh(model, train_ds, val_ds, cfg):
    print("\n" + "=" * 60)
    print(f"  TRAINING CONFIG F: Multi-Scale + Mean-Pool + Bi + Evi")
    print("=" * 60)
...
def evaluate_soh(model, val_loader, test_loader, cfg):
    print("\n" + "=" * 60)
    print("  SOH EVALUATION - TEST SET (Config F: Multi-Scale + Mean-Pool + Bi + Evi)")
    print("=" * 60)
...
def print_deployment_report(model, cfg):
    print("\n" + "=" * 60)
    print("  MODEL COMPLEXITY & DEPLOYMENT METRICS (Config F)")
    print("=" * 60)
    ...
    print(f"  CNN              : Multi-scale ({cfg['cnn_channels']}, kernels={cfg['cnn_kernels']})")
...
if __name__ == "__main__":
    print("=" * 60)
    print("  CONFIGURATION F: Multi-Scale CNN + Mean-Pooling + Bidirectional + Evidential")
    print("=" * 60)
    print(f"Device: {DEVICE}")
    ...
    print("\nBuilding BEM-SOH model (Config F: Multi-Scale + Mean-Pool + Bi + Evi)...")
    ...
    print("\n" + "=" * 60)
    print("  FINAL RESULTS SUMMARY (Config F)")
    print("=" * 60)
