# config_G.py
# Configuration G: Single-Scale CNN + Mean-Pooling + Bidirectional + Evidential
# 
# Purpose: Tests the combined effect of removing BOTH multi-scale CNN AND attention pooling
# Compares to Config D (Multi-scale + Attention) to isolate the combined contribution
# Compares to Config E (Single-scale + Attention) and Config F (Multi-scale + Mean-pool)
# to see if the effects are additive
#
# Architecture:
#   - CNN Backbone: SINGLE-SCALE (1 branch: 128 channels, kernel=7)
#   - Pooling: MEAN-POOLING (average over sequence dimension)
#   - Encoder: Bidirectional Mamba x3 layers (forward + backward)
#   - UQ Head: Evidential (NIG: gamma, nu, alpha, beta)
#   - Calibration: None
#
# What it isolates:
#   - Combined effect of both simplifications
#   - Is the full multi-scale + attention necessary, or is a minimal backbone enough?
#   - This is the "minimal backbone" control for your ablation
#
# Hypothesis:
#   - If Config G ≈ Config D, the full architecture is over-engineered
#   - If Config G > Config D, both components contribute meaningfully
#   - The degradation should be approximately additive: 
#     (D → G) ≈ (D → E) + (D → F)
...
CFG = dict(
    ...
    save_path = r"C:\Users\admin\Desktop\DR2\16 Contributions\Contr03\MIT_Stanford_TRI_SOH_Estimation\checkpoints\config_G_singlescale_meanpool_bidirectional_evidential_30.pt",
    ...
    # Model
    # ─── CONFIG G: SINGLE-SCALE CNN (like Config E) ───
    cnn_channels = [128],      # Single branch
    cnn_kernels  = [7],        # Single kernel (middle branch)
    d_model      = 128,
    d_state      = 16,
    d_conv       = 4,
    expand       = 2,
    ...
    # CONFIG G: Single-scale CNN + Mean-Pooling + Bidirectional + Evidential
    bidirectional = True,    # ← BIDIRECTIONAL (same as Config D)
    evidential    = True,    # ← EVIDENTIAL (same as Config D)
    calibrate     = False,   # ← NO calibration (raw uncertainty)
    mean_pool     = True,    # ← MEAN-POOLING (like Config F)
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  Model — SINGLE-SCALE CNN + MEAN-POOLING
# ─────────────────────────────────────────────────────────────────────────────

class MultiScaleCNN(nn.Module):
    """Multi-scale CNN — but Config G uses only 1 branch (like Config E)."""
    def __init__(self, input_dim, channels, kernels, dropout=0.1):
        ...
        self.out_dim = sum(channels)  # Config G: 128 (single branch)
...
class BEM_SOH(nn.Module):
    def __init__(self, cfg):
        ...
        cnn_out = sum(C["cnn_channels"])  # Config G: 128
        ...
        # ─── CONFIG G: MEAN-POOLING (like Config F) ───
        self.mean_pool = C.get("mean_pool", False)
    ...
    def encode(self, x):
        ...
        if self.mean_pool:
            ...
            # ─── ATTENTION POOLING (only if mean_pool=False) ───
            # This is kept for compatibility but Config G always uses mean_pool=True
            self.attn_pool = nn.Linear(self.cfg["d_model"], 1)
            attn = F.softmax(self.attn_pool(z), dim=1)
            z_pooled = (z * attn).sum(dim=1)
        
        return z_pooled, attn

    def forward(self, x):
        z_pooled, attn = self.encode(x)
        return self.head(z_pooled), attn
...
def train_soh(model, train_ds, val_ds, cfg):
    print("\n" + "=" * 60)
    print(f"  TRAINING CONFIG G: Single-Scale + Mean-Pool + Bi + Evi (Minimal Backbone)")
    print("=" * 60)
...
def evaluate_soh(model, val_loader, test_loader, cfg):
    print("\n" + "=" * 60)
    print("  SOH EVALUATION - TEST SET (Config G: Single-Scale + Mean-Pool + Bi + Evi)")
    print("=" * 60)
...
def print_deployment_report(model, cfg):
    print("\n" + "=" * 60)
    print("  MODEL COMPLEXITY & DEPLOYMENT METRICS (Config G)")
    print("=" * 60)
    ...
    print(f"  CNN              : SINGLE-SCALE (kernel=7, channels=128)")
    print(f"  Pooling          : MEAN-POOLING (no learned attention)")
...
if __name__ == "__main__":
    print("=" * 60)
    print("  CONFIGURATION G: Single-Scale CNN + Mean-Pooling + Bidirectional + Evidential")
    print("  (Minimal Backbone Control)")
    print("=" * 60)
    print(f"Device: {DEVICE}")
    ...
    print("\nBuilding BEM-SOH model (Config G: Single-Scale + Mean-Pool + Bi + Evi)...")
    ...
    print("\n" + "=" * 60)
    print("  FINAL RESULTS SUMMARY (Config G)")
    print("=" * 60)
