# train_soh_config_c_single_cnn.py
#
# ARCHITECTURE ABLATION - CONFIG C
# Replace Multi-Scale CNN with Single CNN (kernel=7)
#
# Configuration: Full Model but WITHOUT Multi-Scale CNN
# - Multi-Scale CNN: ✗ (replaced with Single CNN kernel=7)
# - Mamba Encoder: ✓
# - Attention Pooling: ✓
...
CFG = dict(
    ...
    save_path = r"C:\Users\admin\Desktop\DR2\16 Contributions\Contr03\MIT_Stanford_TRI_SOH_Estimation\checkpoints\soh_config_c_single_cnn.pt",
    ...
    # Model - Single CNN instead of Multi-Scale
    # cnn_channels: single branch instead of [32, 64, 128]
    # cnn_kernels: single kernel=7 instead of [3, 7, 15]
    single_cnn_channels = 64,  # Single branch with 64 channels
    single_cnn_kernel = 7,     # Medium kernel size
    cnn_channels = [64],       # Single branch
    cnn_kernels = [7],         # Single kernel
    d_model = 128,
    d_state = 16,
    d_conv = 4,
    expand = 2,
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  Model - CONFIG C: Single CNN (kernel=7) instead of Multi-Scale
# ─────────────────────────────────────────────────────────────────────────────

class SingleCNN(nn.Module):
    """
    CONFIG C: Single CNN branch instead of Multi-Scale
    Uses a single convolutional branch with kernel=7
    """
    def __init__(self, input_dim, channels, kernel, dropout=0.1):
        super().__init__()
        # Single branch with kernel=7
        self.branch = nn.Sequential(
            nn.Conv1d(input_dim, channels, kernel_size=kernel, padding=kernel//2, bias=False),
            nn.BatchNorm1d(channels), nn.GELU(),
            nn.Conv1d(channels, channels, kernel_size=kernel, padding=kernel//2, bias=False),
            nn.BatchNorm1d(channels), nn.GELU(),
        )
        self.out_dim = channels  # Same as the single branch output
...
class CNNMambaSOH_ConfigC(nn.Module):
    """
    CONFIG C: Single CNN instead of Multi-Scale CNN
    """
    def __init__(self, cfg):
        super().__init__()
        C = cfg
        
        # ⭐ CONFIG C CHANGE: Single CNN instead of Multi-Scale
        self.cnn = SingleCNN(
            input_dim=C["input_dim"],
            channels=C["single_cnn_channels"],
            kernel=C["single_cnn_kernel"],
            dropout=C["dropout"]
        )
        cnn_out = self.cnn.out_dim
        
        # Projection (SAME as full model)
    ...
    def encode(self, x):
        # ⭐ CONFIG C: Single CNN instead of Multi-Scale
...
def train_soh(model, train_ds, val_ds, cfg):
    print("\n" + "="*60)
    print("  TRAINING SOH - CONFIG C (Single CNN)")
    print("="*60)
...
def evaluate_soh(model, test_ds, cfg):
    print("\n" + "="*60)
    print("  SOH EVALUATION — TEST SET (Config C - Single CNN)")
    print("="*60)
...
def print_model_summary(model, cfg):
    ...
    print(f"  CNN: Single branch (channels={cfg['single_cnn_channels']}, kernel={cfg['single_cnn_kernel']})")
    print(f"  d_model: {cfg['d_model']}  | Mamba layers: {cfg['n_mamba_layers']}")
    print(f"  ⚙️  Config C: Single CNN instead of Multi-Scale")


if __name__ == "__main__":
    print("="*60)
    print("  CONFIG C: REPLACE MULTI-SCALE CNN WITH SINGLE CNN")
    print("  (Single branch, kernel=7)")
    print("="*60)
    print(f"Device: {DEVICE}")
    ...
    # Build model - CONFIG C
    print("\nBuilding CNN-Mamba-UQ model - Config C (Single CNN)...")
    model = CNNMambaSOH_ConfigC(CFG).to(DEVICE)
    ...
    torch.save({
        ...
        "results": results,
        "history": history,
        "config": "Config C - Single CNN (kernel=7)"
    }, CFG["save_path"])
