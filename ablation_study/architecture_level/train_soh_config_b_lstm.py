# train_soh_config_b_lstm.py
#
# ARCHITECTURE ABLATION - CONFIG B
# Replace Mamba Encoder with 2-Layer LSTM
#
# Configuration: Full Model but WITHOUT Mamba (replaced with LSTM)
# - Multi-Scale CNN: ✓
# - Mamba Encoder: ✗ (replaced with 2-layer LSTM)
# - Attention Pooling: ✓
...
CFG = dict(
    ...
    save_path = r"C:\Users\admin\Desktop\DR2\16 Contributions\Contr03\MIT_Stanford_TRI_SOH_Estimation\checkpoints\soh_config_b_lstm.pt",
    ...
    # Model - IDENTICAL to full model EXCEPT Mamba replaced with LSTM
    cnn_channels = [32, 64, 128],
    cnn_kernels = [3, 7, 15],
    d_model = 128,
    # Mamba-specific params (not used but kept for compatibility)
    d_state = 16,
    d_conv = 4,
    expand = 2,
    n_mamba_layers = 3,  # Will use 2 LSTM layers instead
    dropout = 0.15,
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  Model - CONFIG B: Replace Mamba with 2-Layer LSTM
# ─────────────────────────────────────────────────────────────────────────────
...
class LSTMEncoder(nn.Module):
    """
    CONFIG B: LSTM Encoder replacing Mamba
    2-layer LSTM with the same d_model dimension
    """
    def __init__(self, d_model, num_layers=2, bidirectional=False, dropout=0.15):
        super().__init__()
        ...
        # Projection back to d_model if bidirectional
        if bidirectional:
    ...
    def forward(self, x):
        ...
        lstm_out, _ = self.lstm(x)  # (batch, seq_len, d_model * (2 if bidirectional else 1))
        lstm_out = self.proj(lstm_out)  # (batch, seq_len, d_model)
        lstm_out = self.norm(lstm_out)
        return self.dropout(lstm_out)


class CNNMambaSOH_ConfigB(nn.Module):
    """
    CONFIG B: Replace Mamba with 2-Layer LSTM
    """
    def __init__(self, cfg):
        super().__init__()
        C = cfg
        
        # CNN (SAME as full model)
        ...
        # ⭐ CONFIG B CHANGE: LSTM instead of Mamba
        self.encoder = LSTMEncoder(
            d_model=C["d_model"],
            num_layers=C["lstm_num_layers"],
            bidirectional=C["lstm_bidirectional"],
            dropout=C["dropout"]
        )
    ...
    def encode(self, x):
        ...
        # ⭐ CONFIG B: LSTM instead of Mamba
        z = self.encoder(z)
...
def train_soh(model, train_ds, val_ds, cfg):
    print("\n" + "="*60)
    print("  TRAINING SOH - CONFIG B (LSTM instead of Mamba)")
    print("="*60)
...
def evaluate_soh(model, test_ds, cfg):
    print("\n" + "="*60)
    print("  SOH EVALUATION — TEST SET (Config B - LSTM)")
    print("="*60)
...
def print_model_summary(model, cfg):
    ...
    print(f"  ⚙️  Encoder: 2-Layer LSTM (replacing Mamba)")
    print(f"  ⚙️  Pooling: Attention Pooling (kept)")


if __name__ == "__main__":
    print("="*60)
    print("  CONFIG B: REPLACE MAMBA WITH 2-LAYER LSTM")
    print("="*60)
    print(f"Device: {DEVICE}")
    ...
    # Build model - CONFIG B
    print("\nBuilding CNN-Mamba-UQ model - Config B (LSTM instead of Mamba)...")
    model = CNNMambaSOH_ConfigB(CFG).to(DEVICE)
    ...
    torch.save({
        ...
        "results": results,
        "history": history,
        "config": "Config B - LSTM instead of Mamba"
    }, CFG["save_path"])
