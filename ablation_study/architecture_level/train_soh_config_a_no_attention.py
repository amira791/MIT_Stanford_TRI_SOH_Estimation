# train_soh_config_a_no_attention.py
#
# ARCHITECTURE ABLATION - CONFIG A
# Replace Attention Pooling with Global Average Pooling
#
# Configuration: Full Model but WITHOUT Attention Pooling
# - Multi-Scale CNN: ✓
# - Mamba Encoder: ✓
...
CFG = dict(
    ...
    save_path = r"C:\Users\admin\Desktop\DR2\16 Contributions\Contr03\MIT_Stanford_TRI_SOH_Estimation\checkpoints\soh_config_a_no_attention.pt",
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  Model - CONFIG A: No Attention Pooling (Global Average Pooling)
# ─────────────────────────────────────────────────────────────────────────────
...
class CNNMambaSOH_ConfigA(nn.Module):
    """
    CONFIG A: No Attention Pooling
    Replaces attention pooling with Global Average Pooling
    """
    def __init__(self, cfg):
        ...
        # ⭐ CONFIG A CHANGE: Global Average Pooling instead of Attention
        # No attn_pool layer needed - we'll use mean pooling in encode()
    ...
    def encode(self, x):
        ...
        z = self.mamba(z)
        # ⭐ CONFIG A CHANGE: Global Average Pooling (mean over time dimension)
        # Instead of: attn = F.softmax(self.attn_pool(z), dim=1); return (z * attn).sum(dim=1)
        return z.mean(dim=1)  # Global Average Pooling
...
def train_soh(model, train_ds, val_ds, cfg):
    print("\n" + "="*60)
    print("  TRAINING SOH - CONFIG A (No Attention)")
    print("="*60)
...
def evaluate_soh(model, test_ds, cfg):
    print("\n" + "="*60)
    print("  SOH EVALUATION — TEST SET (Config A)")
    print("="*60)
...
def print_model_summary(model, cfg):
    ...
    print(f"  ⚙️  Pooling: Global Average Pooling (No Attention)")


if __name__ == "__main__":
    print("="*60)
    print("  CONFIG A: NO ATTENTION POOLING")
    print("  (Replacing Attention with Global Average Pooling)")
    print("="*60)
    print(f"Device: {DEVICE}")
    ...
    # Build model - CONFIG A
    print("\nBuilding CNN-Mamba-UQ model - Config A (No Attention)...")
    model = CNNMambaSOH_ConfigA(CFG).to(DEVICE)
    ...
    torch.save({
        ...
        "results": results,
        "history": history,
        "config": "Config A - No Attention (Global Average Pooling)"
    }, CFG["save_path"])
