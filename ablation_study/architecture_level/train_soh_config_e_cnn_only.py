# train_soh_config_e_cnn_only.py
#
# ARCHITECTURE ABLATION - CONFIG E
# Remove Mamba and Attention Pooling - CNN only
#
# Configuration: Full Model but WITHOUT Mamba and Attention
# - Multi-Scale CNN: ✓
# - Mamba Encoder: ✗ (completely removed)
# - Attention Pooling: ✗ (replaced with Global Average Pooling)
#
# This tests whether the CNN alone can capture both local patterns
# and long-range dependencies for SOH estimation.
...
CFG = dict(
    ...
    save_path = r"C:\Users\admin\Desktop\DR2\16 Contributions\Contr03\MIT_Stanford_TRI_SOH_Estimation\checkpoints\soh_config_e_cnn_only.pt",
    ...
    # Model - CNN only (no Mamba, no Attention)
    cnn_channels = [32, 64, 128],
    cnn_kernels = [3, 7, 15],
    d_model = 128,  # Kept for consistency, but Mamba removed
    dropout = 0.15,
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  Model - CONFIG E: CNN Only (No Mamba, No Attention)
# ─────────────────────────────────────────────────────────────────────────────
...
class CNNOnlySOH(nn.Module):
    """
    CONFIG E: CNN Only - No Mamba, No Attention
    Uses Multi-Scale CNN for feature extraction, then Global Average Pooling
    """
    def __init__(self, cfg):
        super().__init__()
        C = cfg
        
        # Multi-Scale CNN (SAME as full model)
        self.cnn = MultiScaleCNN(C["input_dim"], C["cnn_channels"],
                                 C["cnn_kernels"], C["dropout"])
        cnn_out = sum(C["cnn_channels"])
        
        # ⭐ CONFIG E: No Mamba, No Attention
        # Direct projection from CNN output to feature vector
    ...
    def encode(self, x):
        ...
        z = self.cnn(x)  # (batch, seq_len, cnn_out)
        
        # ⭐ CONFIG E: No Mamba - direct projection and global average pooling
        z = self.cnn_proj(z)  # (batch, seq_len, d_model)
        
        # Global Average Pooling across time dimension
        # Instead of: Mamba + Attention pooling
        return z.mean(dim=1)  # (batch, d_model)
...
def train_soh(model, train_ds, val_ds, cfg):
    print("\n" + "="*60)
    print("  TRAINING SOH - CONFIG E (CNN Only, No Mamba)")
    print("="*60)
...
def evaluate_soh(model, test_ds, cfg):
    print("\n" + "="*60)
    print("  SOH EVALUATION — TEST SET (Config E - CNN Only)")
    print("="*60)
...
def print_model_summary(model, cfg):
    ...
    print(f"  ⚙️  Config E: CNN ONLY (No Mamba, No Attention)")
    print(f"  ⚙️  Pooling: Global Average Pooling")


if __name__ == "__main__":
    print("="*60)
    print("  CONFIG E: REMOVE MAMBA AND ATTENTION")
    print("  (CNN Only - No Mamba, No Attention)")
    print("="*60)
    print(f"Device: {DEVICE}")
    ...
    # Build model - CONFIG E
    print("\nBuilding CNN-Only model - Config E (No Mamba)...")
    model = CNNOnlySOH(CFG).to(DEVICE)
    ...
    torch.save({
        ...
        "results": results,
        "history": history,
        "config": "Config E - CNN Only (No Mamba, No Attention)"
    }, CFG["save_path"])
