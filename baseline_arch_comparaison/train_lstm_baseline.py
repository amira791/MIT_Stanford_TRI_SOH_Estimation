...
def create_sequences(df, window_size=50, stride=2):
    ...
    for cell_id, cell_df in df.groupby("cell_id"):
        ...
        for end in range(window_size, len(X) + 1, stride):
            ...
            X_list.append(X[start:end])  # Keep as sequence (50, 10)
...
def normalize_features(X_train, X_val, X_test):
    ...
    # Reshape to (n_samples, n_features) for scaling
...
def plot_results(y_true, y_pred, save_path):
    ...
    plt.title(f"LSTM: Predictions vs True SOH\nMAE: {mae:.4f}%, R²: {r2:.4f}")
    plt.legend()
    plt.tight_layout()
...
def main():
    print("="*60)
    print("LSTM BASELINE FOR BATTERY SOH ESTIMATION")
    print("="*60)
    ...
    # Save model
    print("\n[4] Saving model...")
    torch.save(model.state_dict(), SAVE_DIR / "lstm_model.pt")
    print(f"  Model saved to: {SAVE_DIR / 'lstm_model.pt'}")
    ...
    # Save results
    # Create a JSON-serializable config copy
    config_serializable = CFG.copy()
    config_serializable["device"] = str(config_serializable["device"])  # Convert device to string
    ...
    "n_params": n_params,
    "config": config_serializable,  # ← Use the serializable version
    }
